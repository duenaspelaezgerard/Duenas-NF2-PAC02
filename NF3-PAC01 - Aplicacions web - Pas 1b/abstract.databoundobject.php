...
abstract class DataBoundObject {
   ...
   public function Load() {     
      ...
      if (isset($this->ID)) {
         ...
         $log->logMessage(" VOY A SELECCIONAR ESTE ID " .$this->ID, Logger::INFO);
         $strQuery = "SELECT ";
   ...
   public function Save() {
      ...
      if (isset($this->ID)) {
         $log = Logger::getInstance();
         $log->logMessage(" VOY A GUARDAR ESTE ID" .$this->ID, Logger::INFO);
   ...
   public function __destruct() {
      if (isset($this->ID)) {   
         if ($this->blForDeletion == true) {
            $log = Logger::getInstance();
            $log -> logMessage(" VOY A SELECCIONAR PARA DESTRUIR ESTE ID" .$this->ID, Logger::NOTICE);
   ...
   private function SetAccessor($strMember, $strNewValue) {
      ...
      if (property_exists($this, $strMember)) {
         $log = Logger::getInstance();
         $log->logMessage(" VOY A DECLARAR ESTO " .$strNewValue, Logger::CRITICAL);
   ...
   private function GetAccessor($strMember) {
      ...
      if (property_exists($this, $strMember)) {
         $log = Logger::getInstance();
         $log->logMessage(" VOY A OBTENER ESTO " .$strMember,  Logger::NOTICE);
